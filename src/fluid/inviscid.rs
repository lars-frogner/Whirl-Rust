//! State and dynamics of an inviscid fluid.
...
/// State of an inviscid fluid.
#[derive(Clone, Debug)]
pub struct InviscidFluid<D: Dim>
where
...
#[derive(Clone, Debug)]
pub struct InviscidTimeDerivatives<D: Dim>
where
    DefaultAllocator: Allocator<D>,
{
    dv_dt: Vec<Vector<D>>,
    ...
    DefaultAllocator: Allocator<D>,
{
    ...
    pub fn new<EOS, K, M, V, E>(
        equation_of_state: &EOS,
        kernel: &K,
        mass_distribution: &M,
        velocity_distribution: &V,
        energy_distribition: &E,
        ...
        DefaultAllocator: Allocator<D>,
    {
        ...
        let specific_energies = energy_distribition
            .compute_specific_energies(&mass_distribution_state, equation_of_state);
        ...
        let correction_scales = vec![0.0; number_of_particles];
    ...
    fn compute_mean_squared_sound_speed<EOS: EquationOfState>(
        ...
        let mut mean_specific_energy: fvar = self.specific_energies.iter().sum();
        mean_specific_energy /= self.specific_energies.len() as fvar;
        equation_of_state.compute_squared_sound_speed(mean_specific_energy)
    }
}

impl<D: Dim> Fluid<D> for InviscidFluid<D>
where
    DefaultAllocator: Allocator<D>,
{
    type TimeDerivatives = InviscidTimeDerivatives<D>;
        ...
        K: Kernel<D>,
    {
        ...
        self.update_gas_pressures(equation_of_state);
        ...
        for i in 0..self.number_of_particles() {
            ...
            let (dv_a_dt, dv_dt_upper) = time_derivatives
                ...
                .split_first_mut()
                .unwrap();
            let (du_a_dt, du_dt_upper) = time_derivatives
                .du_dt
                .split_at_mut(i)
                .1
                .split_first_mut()
            ...
            for (r_b, v_b, &h_b, &rho_b, &omega_b, &pg_b, dv_b_dt, du_b_dt) in izip!(
                r_upper,
                v_upper,
                h_upper,
                rho_upper,
                ...
                if distance > 0.0 && K::is_nonzero(q_a) || K::is_nonzero(q_b) {
                    ...
                    *du_a_dt += (grad_term_a + diffusion_term) * v_ab_parallel;
                    *du_b_dt += (grad_term_b + diffusion_term) * v_ab_parallel;
                }
            }
        }
    }
        ...
        T: IntoIterator<Item = (fvar, &'a Self::TimeDerivatives)>,
    {
        for (weight, time_derivatives) in weighted_time_derivatives {
            ...
            for (v, u, dv_dt, du_dt) in izip!(
                &mut self.velocities,
                &mut self.specific_energies,
                &time_derivatives.dv_dt,
                &time_derivatives.du_dt
            ) {
                *v += weighted_time_step * dv_dt;
                *u += weighted_time_step * du_dt;
            }
        }
    }
...
impl<D: Dim> InviscidTimeDerivatives<D>
where
    DefaultAllocator: Allocator<D>,
{
    ...
    fn new(number_of_particles: usize) -> Self {
        Self {
            ...
            du_dt: vec![0.0; number_of_particles],
        }
    }
    ...
    fn reset(&mut self) {
        ...
        self.du_dt.iter_mut().for_each(|du_a_dt| *du_a_dt = 0.0);
    }
}

impl<D: Dim> TimeDerivatives<D> for InviscidTimeDerivatives<D> where DefaultAllocator: Allocator<D> {}
